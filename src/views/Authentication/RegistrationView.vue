<script setup lang="ts">
import { TitleText, SpinnerCircle, AppInput } from "@components";
import { ref } from "vue";
import { validateEmail } from "@/helpers/validateEmail";
import {
  Auth,
  createUserWithEmailAndPassword,
  updateProfile,
  getAuth,
  UserCredential,
} from "firebase/auth";
import { useRouter } from "vue-router";
import { foodImg } from "@assets/img";

const auth: Auth | null = getAuth();
const router = useRouter();

let email = ref<string>("");
let password = ref<string>("");
let fullName = ref<string>("");
let loading = ref<boolean>(false);

const onSignUp = async (): Promise<void> => {
  let isValidEmail = validateEmail(email.value);
  if (!isValidEmail) {
    alert("Please enter a valid email address");
    return;
  }
  loading.value = true;
  try {
    let userCredential: UserCredential = await createUserWithEmailAndPassword(
      auth,
      email.value,
      password.value
    );
    const user = userCredential.user;
    if (user) {
      await updateProfile(user, { displayName: fullName.value || "Cool User" });
      router.push({ name: "home" });
      user.reload();
    }
  } catch (error: any) {
    alert(
      `Failed to create account because of error: ${error.code} ${error.message}`
    );
  } finally {
    loading.value = false;
  }
};
</script>

<template>
  <div class="flex items-center justify-center h-screen md:p-8">
    <div
      class="flex flex-col lg:w-1/2 xl:w-1/3 w-full h-full items-center justify-start md:justify-center bg-orange-400 md:p-20 rounded-xl md:shadow-lg p-8"
    >
      <div class="flex flex-row self-start items-center mb-14">
        <img :src="foodImg" alt="logo" class="w-20 h-20 mr-5 object-contain" />
        <TitleText
          class="text-white text-center mb-0"
          style="text-shadow: rgba(0, 0, 0, 0.4) 2px 2px 2px"
        >
          Meal Finder App
        </TitleText>
      </div>
      <TitleText class="text-white self-start">Create Account:</TitleText>
      <h3 class="text-white self-start mb-8">Let's get started!</h3>

      <AppInput
        type="text"
        placeholder="Full Name"
        v-model="fullName"
        class="mb-5"
      />
      <AppInput type="email" placeholder="Email" v-model="email" class="mb-5" />
      <AppInput type="password" placeholder="Password" v-model="password" />

      <button
        @click="onSignUp"
        type="submit"
        :disabled="!email || !password || !fullName"
        class="flex justify-center items-center my-8 rounded w-full h-14 bg-white text-orange-500 text-center hover:opacity-80 disabled:opacity-80 disabled:cursor-not-allowed hover:cursor-pointer"
      >
        <SpinnerCircle v-if="loading" />
        <span v-else>Create Account</span>
      </button>
      <div>
        <router-link
          :to="{ name: 'login' }"
          class="text-white hover:underline hover:cursor-pointer"
          >Have an account? Login</router-link
        >
      </div>
    </div>
  </div>
</template>
